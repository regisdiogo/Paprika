...
class App {
	...
	private function renderPage($page, $template = null, $var = null) {
		...
		if (isset($page)) {
			if (!file_exists(Config::getWebIncludePagePath().$page)) {
				throw new \Exception('Não existe o arquivo - '.$page);
			} else {
		...
		if ($template) {
			...
			if (preg_match('/<\?\/\/{PPK-USE-INSIDE-TEMPLATE-FILE:"(?<value>.+?)"}\?>/', $fileContent, $matches, PREG_OFFSET_CAPTURE)) {
			...
			$c[\presentation\BasePresentation::TEMPLATE_INNER_FILE] = $pages;
			require_once($template);
		} else {
			require_once(Config::getWebIncludePagePath().$page);
		}
	}
	...
	private function loadPaprikaClass($className) {
		...
		foreach ($namespaces as $namespace) {
			...
			$classFile = PAPRIKA_PATH.'/'.$className.'.php';
	...
	private function loadCustomCodeClass($className) {
		$srcFolder = realpath(Config::CUSTOM_CODE_FOLDER).'/';
...
function p($object, $title = '') {
	echo "<div align='left' style='font-size:11px;border:1px solid gray'><pre>";
